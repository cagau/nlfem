...
def main():
    ...
    for n in conf.N:
        ...
        h = 12/n/10
        mesh = RegMesh1D(conf.a, conf.b, conf.delta, h, 0)
        ...
        g = np.apply_along_axis(conf.u_exact, 0, mesh.vertices[mesh.K_Omega:])
        f -= A_I@g
        ...
        # Refine to N_fine ----------------------------------------------------------------
        mesh.plot_ud(pp)
        h_fine = 12/conf.N_fine
        mesh = RegMesh1D(conf.a, conf.b, conf.delta, h_fine, 0, ufunc=conf.u_exact, coarseMesh=mesh,
