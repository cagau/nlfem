...
quad_order_outer = 8#'test'#'new'#8 # choose from [8, 5, 3, 2, 'new']
quad_order_inner = 5
...
#    hash_table_nodes = [list(set(np.where(norm((verts[nodes]-np.repeat(verts[nodes[i]][:,np.newaxis], len(nodes), axis = 1).transpose()).transpose())<=(eps+diam))[0].tolist())&set(range(i+1))) for i in range(len(nodes))]
    ...
    #------------------------------------------------------------------------------  
    """ BUILDING MESH CLASS """
    proc_mesh_data = [triangles, omega, verts, hash_table, boundary, nodes, NHD, max(diameter), support, hash_table_approx, bary, bdry_nodes, hash_table_bary, interface, lines, len(verts), len(triangles), len(omega), len(nodes_inner)]
...
#    hash_table_nodes = [list(set(np.where(norm((verts[nodes]-np.repeat(verts[nodes[i]][:,np.newaxis], len(nodes), axis = 1).transpose()).transpose())<=(eps+diam))[0].tolist())&set(range(i+1))) for i in range(len(nodes))]
    ...
    #------------------------------------------------------------------------------  
    """ BUILDING MESH CLASS """
    proc_mesh_data = [triangles, omega, verts, hash_table, boundary, nodes, NHD, diam, support, hash_table_approx, bary, bdry_nodes, hash_table_bary, interface,[],len(verts), len(triangles), len(omega), len(nodes_inner)]
    mesh = Mesh(proc_mesh_data)
    #------------------------------------------------------------------------------       
    return mesh, proc_mesh_data
...
if quad_order_outer == 'test':
    ...
    P = np.array([[0.16666666666667,    0.16666666666667],
                  [0.16666666666667,    0.66666666666667],  
                  [0.66666666666667,    0.16666666666667]  ]).transpose()
...
if quad_order_inner == 8:
    ...
    P2 = np.array([[0.16666666666667,    0.16666666666667],
                   [0.16666666666667,    0.66666666666667],  
                   [0.66666666666667,    0.16666666666667]  ]).transpose()
...
P_outer = np.array([[0.33333333, 0.33333333],
               [0.2, 0.6],
               [0.2, 0.2],
               [0.6, 0.2]]).transpose()
weights_outer = np.array([-27. / 48, 25. / 48, 25. / 48, 25. / 48])
P_inner = np.array([[0.16666666666667, 0.16666666666667],
               [0.16666666666667, 0.66666666666667],
               [0.66666666666667, 0.16666666666667]]).transpose()
weights_inner = 1. / 3 * np.ones(3)
n_outer, n_inner = 4, 3
X_outer, Y_inner = np.tile(P_outer,n_inner), np.repeat(P_inner,n_outer, axis=1)
W_minprec = 0.25 * np.array([weights_outer[r]*weights_inner[i] for i in range(n_inner) for r in range(n_outer)])
...
#    labels_domains = np.sort(np.unique(mesh.triangles[:,0]))
    ...
    def aux(m):
        ...
        for i in pieces[m].tolist():
            ...
            #if (i==10):
            #    print("Totalocal")
            termTotalLocal = np.zeros((3, 3))
            termTotalNonloc = np.zeros((3, 3))
            ...
            for j in hash_i:
                ...
                if norm(mesh.bary[i_triangles]-mesh.bary[j]) < eps_i -mesh.diam:
                    ...
                    for a in range(3):
                        ...
                        for b in range(3):
#                           if label_j != labels_domains[-1]:
                            termNonloc[a, b] += -det_T_i * abs(det_T_j) * ( PSI_X[a] *  PSI_Y[b]  * W * gam_j(Phi_i(X),Phi_j(Y))).sum()
                            termTotalNonloc[a,b] += -det_T_i * abs(det_T_j) * ( PSI_X[a] *  PSI_Y[b]  * W * gam_j(Phi_i(X),Phi_j(Y))).sum()
                            L[kk, T_j[b]] += -det_T_i * abs(det_T_j) * ( PSI_X[a] *  PSI_Y[b]  * W * gam_j(Phi_i(X),Phi_j(Y))).sum()
                            termLocal[a,b] += det_T_i * abs(det_T_j) * ( PSI_X[a] *  PSI_X[b]  * W * gam_j(Phi_i(X),Phi_j(Y))).sum()
                            termTotalLocal[a,b] += det_T_i * abs(det_T_j) * ( PSI_X[a] *  PSI_X[b]  * W * gam_j(Phi_i(X),Phi_j(Y))).sum()
                ...
                #                plt.gca().add_patch(plt.Polygon(tri , closed=True, fill = False))
                    ...
                    for a in range(3):
                        kk = T_i[a]
                        for b in range(3):
                            termNonloc[a, b] += -det_T_i * (PSI_P[a] * I[b] * weights).sum()
                            termTotalNonloc[a, b]  += -det_T_i * (PSI_P[a] * I[b] * weights).sum()
                            L[kk, T_j[b]] += -det_T_i * (PSI_P[a] * I[b] * weights).sum()

                            termLocal[a,b] += det_T_i * (PSI_P[a] * PSI_P[b] * I[3] * weights).sum()
                            termTotalLocal[a,b] += det_T_i * (PSI_P[a] * PSI_P[b] * I[3] * weights).sum()
                            L[kk, T_i[b]] += det_T_i * (PSI_P[a] * PSI_P[b] * I[3] * weights).sum()
                    if False and ((i==16)   and (j in [42])):
                        I = np.array(list(map(I1, P.transpose()))).transpose()
                        res = I1(P[:,15])

                        # Test Plot
                        #john_transP = (T_i_v[0, np.newaxis].T + Mat_i.dot(P)).T
                        #plt.scatter(john_transP[:,0], john_transP[:,1])
                        #for k in range(len(P.T)):
                        #    john_xtest_plot = (T_i_v[0] + Mat_i.dot(P[:,k]))
                        #    plt.annotate(k, john_xtest_plot)
                        #plt.show()

                        john_xtest = (T_i_v[0] + Mat_i.dot(P[:, 15]))
                        john_tris = retriangulate(john_xtest, T_j_v, norm, eps_i )
                        john_tris = np.array(john_tris)
                        #if i==7:
                        fig = plt.figure()  # create a figure object
                        ax = fig.add_subplot(1, 1, 1)
                        plt.gca().set_aspect('equal')
                        ax.set_title(j)
                        for tri in john_tris:
                            tri = np.array(tri)
                            ax.fill(tri[:, 0], tri[:, 1], color="r", alpha=.3)
                            ax.fill(tri[:, 0], tri[:, 1], color="r", fill=False, lw=.1)
                        #plt.show()
                        if j==42:
                            plt.show()
                            print("Computed Terms")
